...
`test/functional/test_framework/test_framework.py:LISYNetworkTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/lisynetwork_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_lisynetworkd = config["components"].getboolean("ENABLE_LISYNETWORKD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/LISYNetworkProject/LISYNetwork/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/LISYNetworkProject/LISYNetwork/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_lisynetworkd):
        print("No functional tests to run. Wallet, utils, and lisynetworkd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if lisynetworkd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "lisynetworkd"]) is not None:
            print("%sWARNING!%s There is already a lisynetworkd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "LISYNETWORKD" not in os.environ:
        os.environ["LISYNETWORKD"] = build_dir + '/src/lisynetworkd' + exeext
        os.environ["LISYNETWORKCLI"] = build_dir + '/src/lisynetwork-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie lisynetworkds, we can apply a
...
class RPCCoverage():
    ...
    commands per `lisynetwork-cli help` (`rpc_interface.txt`).
