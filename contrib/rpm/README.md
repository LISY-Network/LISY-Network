...
The RPM spec file provided here is for LISYNetwork-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://lisynetwork.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[LISYNetwork git repository](https://github.com/LISYNetworkProject/LISYNetwork) but are not part of
...
Currently only `Source10` is used, for the example `lisynetwork.conf` file.
...
in the lisynetwork git repository. At present only an SVG version of the LISYNetwork
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the lisynetwork
...
needed when 0.12.1 is released, a proper fix is already in the LISYNetwork git
master branch.
...
idea to build LISYNetwork Core against a static version of that library compiled
...
The `lisynetwork.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `lisynetwork-server` package. At this point, most RPM based
...
The `lisynetwork-server` package should have SELinux support. How to properly do
...
LISYNetwork-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/lisynetwork/). He has been
packaging LISYNetwork for Fedora at least since 2012.
...
4. I renamed the `lisynetwork` package that contains the Qt GUI to `lisynetwork-core` as
that appears to be how the general population refers to it, in contrast to
`lisynetwork-xt` or `lisynetwork-classic`. I wanted to make sure the general population
