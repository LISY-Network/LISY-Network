 ...
 * between lisynetworkd, and lisynetwork-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("lisynetwork-shutoff");
    ...
    FlushWallets();
#endif
    GenerateLISYNetworks(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), LISYNETWORK_CONF_FILENAME));
    if (mode == HMM_LISYNETWORKD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), LISYNETWORK_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/LISYNetworkProject/LISYNetwork>";
    const std::string URL_WEBSITE = "<https://lisynetwork.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("lisynetwork-loadblk");
...
/** Sanity checks
 *  Ensure that LISYNetwork is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LISYNetwork version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single LISYNetwork process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", LISYNETWORK_CONF_FILENAME)).string());
    ...
    GenerateLISYNetworks(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
