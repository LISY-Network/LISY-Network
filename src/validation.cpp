...
#if defined(NDEBUG)
# error "LISYNetwork cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "LISYNetwork Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s LISY additional fees, %d delta bytes\n",
    ...
    if (halvings >= 64)
{
	...
	{
        nSubsidy = 2079000 * COIN;
	}
    else if(nHeight > 10 && nHeight <= 52500010)
    {
        nSubsidy = 0.004 * COIN;
    }
       return nSubsidy;
...
void ThreadScriptCheck() {
    RenameThread("lisynetwork-scriptch");
